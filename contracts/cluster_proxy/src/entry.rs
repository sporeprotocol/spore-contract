...
use spore_utils::{find_position_by_type, find_position_by_lock, find_position_by_type_arg, verify_type_id, find_position_by_type_arg_unpack};
...
fn process_creation(index: usize) -> Result<(), Error> {
    ...
    let cell_dep_index = find_position_by_type_arg_unpack(target_cluster_id.as_slice(), CellDep, Some(is_valid_cluster_cell)).ok_or(Error::ClusterCellNotInDep)?;
    ...
    return if find_position_by_type_arg(&target_cluster_id, Input, Some(is_valid_cluster_cell)).is_some()
        && find_position_by_type_arg(&target_cluster_id, Output, Some(is_valid_cluster_cell)).is_some() {
        Ok(())
    } else {
...
fn process_transfer() -> Result<(), Error> {
    ...
    if input_proxy_type.args().as_slice()[0..CLUSTER_PROXY_ID_LEN] != output_proxy_type.args().as_slice()[0..CLUSTER_PROXY_ID_LEN] {
